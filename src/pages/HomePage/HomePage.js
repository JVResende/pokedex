...
function HomePage() {
  ...
  const { nameNumberQuery, typeQuery, selectedHome, setSelectedHome, isLoading, error, pokemonsData, playPcOn, playAPress, listPageNumber, setListPageNumber, playCompletedDex, storedDex } = useContext(GlobalStateContext)
  ...
  if (displayCards.length === 0 && nameNumberQuery === "" && typeQuery === "") {
    displayCards = <GotEmAll><div className="message"><h2>You Got 'Em All!</h2><h2>(Desmute o som!)</h2></div> <ReactPlayer width='auto' playing={true} muted={true} controls url='https://www.youtube.com/watch?v=atqKPe8lOpE' /></GotEmAll>
  }
